...
<Note>
Not yet on Maven Central — the Gradle publish config (POM metadata, signing) is already in place,
it just needs a verified Sonatype account for the `com.personaliai` namespace. Once that happens,
the dependency becomes `implementation("com.personaliai:chatty-android-sdk:1.0.1")` instead.
</Note>
